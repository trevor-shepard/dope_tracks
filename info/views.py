...
from .serializers import GroupSerializer, TrackSerializer
...
def test(request):
  ...
  history = UserTrackHistory.objects.filter(reduce(operator.or_, users)).order_by('-played_on')
...
def get_lindsey(request):
  response = requests.get(build_lastfm_api_call(user='ak0716', method='user.getRecentTracks'))
  response.raise_for_status()
  initial_data = response.json()
...
#   def retrieve(self, request, pk=None): #why does pk=none?
#     queryset = Group.objects.all()
